...
void compute_score_row_sad(cv::Mat left, cv::Mat right, int element_max, int index_left,
    ...
    for (int r_col = 0; r_col < element_max; r_col++) {
        // cout << (int)right.data[index_right] << ' ';
        int diff = (left.data[index_left++] & 0xFF) - (right.data[index_right++] & 0xFF);
        // cout << diff << ' ' << (int)left.data[index_left-1] << ' ' << (int)right.data[index_right-1] << endl;
...
void compute_score_row(cv::Mat left, cv::Mat right, int row, int *scores, int minDisparity,
    ...
    for (int d = minDisparity; d < maxDisparity; d++) {
        ...
        // print_array(element_score, col_max);
        ...
        // print_array(element_score, left.cols);
        // if (d==1) {
        //     exit(1);
        // }
...
void func(cv::Mat left, cv::Mat right, int minDisparity, int maxDisparity, int radiusX,
    ...
    int range_disparity = maxDisparity - minDisparity;    
    int length_horizontal = left.cols * range_disparity;
    ...
    cv::Mat horizontal_score = cv::Mat::zeros(regionHeight, length_horizontal, CV_32SC1);
    cv::Mat vertical_score = cv::Mat::zeros(regionHeight, length_horizontal, CV_32SC1);
    ...
    // std::cout << horizontal_score << std::endl;
    // std::cout << first_row << std::endl;
    // print_array(first_row, length_horizontal);
...
int main(int argc, char **argv) {
    ...
    func(img_left_g, img_right_g, 0, max_disp, region_size, region_size, 25, 6, 0.0);
    // cv::imshow("Image", img);
    // cv::waitKey(0);
    return 0;
}